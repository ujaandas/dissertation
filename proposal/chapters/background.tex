...
% Review FSM based masking (outlines, JSON schemas, etc...)
  ...
  Context-Free Grammars (CFGs), such as standard JSON syntax, with negligible
  latency overhead.
  ...
  Scholak et al.~\cite{} introduced PICARD, which parses the top-$k$ beams into
  ...
  Similarly, Syncromesh~\cite{} employs completion engines that query an external
  ...
  neural decoder and symbolic state tracker introduces significant overhead.

  To mitigate these costs, Kogler et al.~\cite{} decouple semantic verification
  ...
  system performs a speculative KV-cache rollback and regenerates the element.
  ...
  Furthermore, Low-Rank Adaptation (LoRA)~\cite{} has proven highly effective for
  ...
  Several recent systems attempt to unify neural predictive models with symbolic
  reasoning engines, combining training-time alignment with inference-time
  verification.

  AlphaGeometry~\cite{} couples a neural language model trained on synthetic
  ...
  symbolic components as an iterative propose-and-verify loop. The language model
  ...
  trace and the model must regenerate the output~\cite{}.
  ...
  A closely related approach, NeSyS~\cite{}, alternates training between a
  ...
  et al.~\cite{} proposed a neuro-symbolic DSL generation framework that combines
